-- Corrected create_family_pledge (v2) — replaces the function in place. Paste this whole
-- file into the Supabase SQL Editor and run it once.
--
-- Fixes BOTH issues found during QA:
--   1. Token: pgcrypto lives in the `extensions` schema on Supabase, so gen_random_bytes
--      is schema-qualified (this SECURITY DEFINER function pins search_path = public).
--   2. recipient_email: the live version was missing recipient_email from the invite
--      INSERT, so §8.2 emails short-circuited with "no recipient". It is included below
--      (last column of the family_pledge_invites insert).
...
begin
  ...
  -- Fix 2: recipient_email IS included (the parent's address when channel = email).
